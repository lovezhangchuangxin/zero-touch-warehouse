...
// 换算、borrow / repay / buy 关键码路径与资金线。与 m3_ops.py 逐行对照，
// 日志序列与世界终态哈希必须一致。
//
...
export function loop() {
  ...
  var milli = function (x) { return Math.floor(x * 1000 + 0.5); };
  var funds = function () {
    Game.log("f:" + milli(Game.gold) + "," + milli(Game.debt));
  };
  ...
  Game.log("sh:" + Game.shelves().length + "," + Game.shelves()[0].capacity);
  Game.log("y2:" + Game.buy("robot", 6, 6)); // 余额 24.9 < 650
  Game.log("b8:" + Game.borrow(700)); // 借款解围
  Game.log("y3:" + Game.buy("robot", 6, 6));
  Game.log("rb:" + Game.robots().length + "," + Game.robots()[1].energy);
