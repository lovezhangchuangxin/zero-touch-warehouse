...
- request_id 宿主进程内单调、跨执行不复位；去重缓存按“当前执行内”分域
  ...
  被拒请求的拒绝结果同样入缓存、预算内重发保证幂等——否则超限后的
  下一执行首个请求会被误判 REQUEST_ID_GAP。
- 完成帧与“已见号”对账（非“已执行号”）：被拒请求见过但未执行。
- 旧执行消息以 EXEC_CLOSED **错误结果**拒绝（宿主可恢复），拒绝号计入
  “已见”对账——被拒后宿主继续发号不得误判 REQUEST_ID_GAP / 完成帧
  LAST_REQUEST_MISMATCH（2026-09-14 评审修复：此前实现漏计，与字段注释
  矛盾）；拒绝不入去重缓存，重发仍先命中执行号检查，天然幂等。旧代次帧
  属协议破坏，终止宿主。
- REQUEST_LIMIT 归脚本级：超限后继续应答错误而不执行，直到宿主自行走
  到终态帧；宿主“正常完成”的结局改判为超限暂停。
- 去重缓存受条目（请求上限 + 1024）与字节（dedup_cache_bytes，默认
  64MB）双上界约束（2026-09-14 评审修复：此前仅条目上界，指纹是帧上限
  1MB 的线上原文、mirror.fetch 的结果是整份镜像，主进程可随请求风暴
  瞬时堆积数百 MB）。字节预算耗尽后跳过缓存新条目——请求照常执行
  应答，该号重发按既有 DUP_REQUEST_MISMATCH 协议故障降级。
