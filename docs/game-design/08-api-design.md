...
Game.vehicles(kind=None)     # 占用装卸位的车辆；kind 传 "in"/"out" 可过滤
Game.market.sell_orders()    # 卖单：对方卖出，玩家买入，车送货来
Game.market.buy_orders()      # 买单：对方收购，玩家卖出，车来取货
...
# 辅助
Game.find_path(start, goal, opts=None)  # 静态障碍最短路径，含 goal 不含 start；不可达返回 None
Game.log(...)                # 输出到日志面板
...
# [(x1, y1), (x2, y2), …]   最短路径，含 goal 不含 start
# []                        已在到达范围内，无需移动
# None                      不可达
```
...
- 纯查询，不消耗行动机会，可用于规划与试探。JS 侧注意 `[]` 为 truthy：判不可达必须用 `path === null`，不能用 `if (path)`。
...
- `r.memory` 是 `Game.memory["robots"][r.id]` 的快捷引用，自动创建，读写同一份数据，跨代码重载持久；其中 `_move` 为 move_to 保留键，玩家不应占用。
...
| 受理 | `OUT_OF_BOUNDS` / `CELL_BLOCKED` | 坐标越界（移动/pick/drop/buy 共用）/ 目标格为静态障碍（建筑、地面货物） |
...
def loop():
    ...
    for r in Game.robots():
        if r.energy < r.energy_max * 0.2 and Game.chargers():
            charger = Game.chargers()[0]
            if r.move_to(charger) == Game.E.ARRIVED:   # 移动不耗电，低电量也回得了桩旁
                r.charge()
    ...
    # 交易策略：有空装卸位且库存低于下限，接最便宜的 A 卖单
    stock = sum(1 for s in Game.shelves()
                for b in s.boxes if b.goods_type == "A")
    stock += sum(1 for b in Game.ground_boxes()
                 if b.goods_type == "A")
    if stock < 2 and any(p.docked_vehicle is None for p in Game.ports()):
