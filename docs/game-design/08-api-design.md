...
6. **金额为定点显示值**：金币、单价、欠款等暴露为普通数字，内部按定点整数存储与判定（见[技术架构 02](../architecture/02-simulation-core.md)）；资金判断勿用浮点等值比较，权威结果以接口返回码为准。
...
- 一切可通行格等代价，返回最短路径之一；BFS 扩展按固定方向序（北、南、西、东）定序，取最先发现的到达集格与其父链路径，相同世界状态下结果确定，便于复现与调试。
- 调用成本受主进程节点预算约束（20 000 次格子扩展，对 50×50 地图留有充分余量）：超限抛出可读错误而非结果码，不可达仍以 `None` / `NO_PATH` 专属表达，不与超限混淆。
...
- 路径、目的地与 opts 缓存于 `r.memory._move`，为 JSON 字符串标量，解析后形如 `{"goal": [x, y], "path": [[x, y], …], "range": n}`（键序随实现的字典序，JSON 对象本身无序）：`path` 为完整轨迹（**含寻路时的起点位置**、含到达格），按机器人实际位置在其中索引推进——结算失败时位置不变、索引不变，天然原路重试。当目的地或 opts 变化、当前位置不在缓存路径上（偏离）、下一步被静态障碍（新建筑、地面货物）占据时重新寻路；结算失败（如 `CELL_CONTESTED`）不使缓存失效，下一 tick 原路重试。缓存只按实际位置推进，不在受理成功时提前消费路径。缓存随 `Game.memory` 跨代码重载与读档保留，不做按 tick 的路径过期；读档后首次调用按偏离规则自动校验，路径过旧即重寻。缓存写入受 memory 容量限额约束，写不下时静默降级为每 tick 重寻，不影响移动本身。`_move` 恒为标量：服务端写它不替换玩家容器节点，宿主 memory 句柄缓存的结构不变量保持成立（见[技术架构 04](../architecture/04-api-bindings.md)）。
...
- ~~`find_path` 与 `move_to` 的调用成本限制；`_move` 缓存格式；是否需要按 tick 的路径过期（类似 Screeps `reusePath`）；是否内建路径可视化~~（已随 M4 落定：节点预算 20 000 次扩展、超限抛可读错误（见「寻路」节）；`_move` 格式与不做按 tick 过期（见「move_to 与 robot.memory」节）；内建路径可视化不做，留画布交互迭代）。
