...
┌───────────▼──────────────────┴──────────────┐
│ 主进程                                       │
│  世界线程：World + tick 状态机 + 受理与结算    │
│  主进程看门狗 ｜ Steam ｜ 存档 IO             │
└───────────┬─────────────────────────────────┘
      loop() 调用与 Game 请求（同步 IPC）
┌───────────▼─────────────────────────────────┐
...
| 进程 | 内容 | 终止后果 |
| --- | --- | --- |
| 主进程 | World、动作受理与结算、存档、Steam、界面 | 自身崩溃则按自动存档恢复（唯一的回档来源） |
| 宿主进程 | 玩家代码执行，同一时间一种语言 | 随时可杀：终止后重启并注入 `Game.memory` 即恢复，世界不受影响 |

- 玩家代码的任何故障都不触及 World：死循环、吞中断、崩溃的最坏损失是普通全局变量，不回档。
- 主进程不依赖任何语言运行时；`Game` 调用与 `loop()` 执行经同步 IPC 往返，单次调用微秒级（机制见 [03](03-player-runtime.md)）。
...
| 线程 | 职责 |
| --- | --- |
| 世界线程 | 独占 `World`，串行执行 tick 状态机；等待 `loop()` 返回期间持续应答宿主进程的 Game 请求 |
...
| `crates/api` | Game 门面定义、快照视图；绑定、IPC 协议与文档的唯一来源（见 [04](04-api-bindings.md)） |
