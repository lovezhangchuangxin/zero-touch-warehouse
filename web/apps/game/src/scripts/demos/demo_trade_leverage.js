// M3 校准 · 杠杆囤货策略：囤货策略 + 借贷放大——现金枯竭时借款吃下
// 低价单，回款充裕时还债。三策略对比之一（records/m3-calibration.md；
// 地图 m3-trade）。

var GOODS = ["water", "battery", "chip"];
// 买卖门深度（相对中枢 mid 的慢速 EMA）：往返税 ≈ 半价差 ×2，须等
// 基准价 ±1σ 量级的深坑 / 高峰才进出场，按货物波动档定深度。
var GATE = {
  water: { buy: 0.93, sell: 1.03 },
  battery: { buy: 0.91, sell: 1.08 },
  chip: { buy: 0.9, sell: 1.12 },
};

var MARGIN = 1.08; // 卖出保底：bid ≥ 摊薄成本 × MARGIN（囤货等更厚的波段）
var MAX_OPEN = 4; // 并发在途订单上限（杠杆吃更多头寸）
var RESERVE = 0; // 杠杆不留现金垫底（缺口靠借款补）
...
function tradePolicy() {
  ...
  for (const o of Game.market.sell_orders()) {
    ...
    if (Game.gold < o.unit_price * o.qty + RESERVE) {
      // 杠杆：机会在门口而现金不足 → 借款补足（给利息留债务余量）。
      if (Game.debt < 700) {
        Game.borrow(200);
      }
...
// 杠杆策略：回款充裕时优先还债（借款在买门触发处就地发生）。
function leveragePolicy() {
  if (Game.debt > 0 && Game.gold > 300) {
    Game.repay(100);
  }
}
